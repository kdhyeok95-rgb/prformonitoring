...
# - [업데이트] 20일 초과 검색 시 1000건 한계 돌파를 위한 스마트 라우터 및 날짜 쪼개기 기능 추가
# -----------------------------------------------------------------------------
...
import urllib.parse
import warnings
import time # 추가됨: 안전한 크롤링을 위한 휴식 시간 부여
from datetime import datetime, date, timedelta # 추가됨: 날짜 쪼개기 연산을 위한 timedelta
...
# ========= [신규 추가] 1000건 한계 돌파를 위한 날짜 쪼개기(Chunking) 엔진 =========
def naver_news_search_by_chunking(query: str, start_dt: date, end_dt: date, limit_per_day: int = 100):
    """
    네이버 API의 1000건 제한을 우회하기 위해, 검색어를 날짜별로 쪼개서 요청하는 스마트 함수입니다.
    """
    ...
    while curr_date <= end_dt:
        # 쿼리에 날짜 문자열을 강제 삽입하여 네이버 검색기가 해당 날짜 위주로 결과를 주도록 유도합니다.
        day_query = f"{query} {curr_date.strftime('%Y-%m-%d')}"
        
        items, start = [], 1
        # 하루당 최대 검색 허용치(limit_per_day)만큼만 가져와서 서버 부하를 방지합니다.
        while len(items) < limit_per_day: 
            display = min(PAGE_SIZE, limit_per_day - len(items))
            params = {"query": day_query, "display": display, "start": start, "sort": "sim"} # 날짜 검색 시 정확도(sim) 정렬이 유리함
            try:
        ...
        # 다음 날짜로 이동 및 사람처럼 보이기 위한 짧은 휴식(서버 차단 방지)
...
# ========= 네이버 뉴스 API (SSL 검증 비활성화 / As-Is 엔진) =========
...
def run(query: str, total: int, out_csv: str | None, whitelist_only: bool,
    ...
    # ... (기존 코드: 명단 불러오고 우호도 불러오는 부분 그대로 유지) ...
    if start_dt or end_dt:
        print(f"[INFO] 날짜 필터: {start_dt or '-'} ~ {end_dt or '-'} (게시일 기준)")

    # ==============================================================
    # 💡 [여기서부터 스마트 라우터 교체 구간] 💡
    # 기존 코드: api_items = naver_news_search(query, total=total, sort=SORT) -> 이 부분을 아래처럼 바꿉니다.
    ...
    # 2. 스마트 스위치 가동
    if start_dt and end_dt and days_diff > 20:
        print(f"[INFO] 수집 기간이 20일을 초과({days_diff}일)했습니다. 1000건 한계 돌파 모드(날짜 쪼개기)를 가동합니다.")
        # 공장 밖에 새로 만들어둘 날짜 쪼개기 함수를 호출합니다.
        api_items = naver_news_search_by_chunking(query, start_dt, end_dt, total) 
    else:
        print(f"[INFO] 수집 기간이 20일 이내입니다. 초고속 API 수집을 가동합니다.")
        # 기존 1000건 이하 초고속 엔진
        api_items = naver_news_search(query, total=total, sort=SORT)
    # ==============================================================

    print(f"[INFO] 네이버 수집 완료: 총 {len(api_items)}건의 기사 확보")

    # works 준비 (이하 코드는 100% 기존과 동일하게 유지됩니다!)
    works = []
    for it in api_items:
        link = it.get("link") or ""
    ...
    for it, url in works:
        ...
        if start_dt or end_dt:
            pd_date = parse_yyyy_mm_dd(pub_date_str) if pub_date_str else None
            if not pd_date:
                continue
            if start_dt and pd_date < start_dt:
                continue
            if end_dt and pd_date > end_dt:
                continue
